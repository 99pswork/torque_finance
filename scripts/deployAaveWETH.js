...
async function deploySimpleETHFactory() {
  const ETHBorrow = await hre.ethers.getContractFactory("AaveWethRefinance");
  let ethBorrow;
  console.log("AaveWethRefinance factory created.");
  try{
      ethBorrow = await ETHBorrow.deploy();
  }
  catch (error) {
    console.error("Error deploying AaveWethRefinance:", error);
    process.exit(1);
  }
  console.log("AaveWethRefinance Contract Address", ethBorrow.target);
  return ethBorrow;
}
