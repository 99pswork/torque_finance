...
contract SimpleUSDTBorrow is SimpleBorrowAbstract {
    using SafeMath for uint256;
    bool firstTimeFlag = true;

    event Borrow(uint supplyAmount, uint borrowAmountUSDC);
    event Repay(uint usdcRepay, uint256 USDTWithdraw);
    ...
    function repay(address _address, uint usdcRepay, uint256 usdtWithdraw) public nonReentrant() {
        require(msg.sender == controller, "Cannot be called directly");
        ...
        callData[0] = supplyAssetCalldata;
        if(usdtWithdraw!=0){
            bytes memory withdrawAssetCalldata = abi.encode(comet, address(this), asset, usdtWithdraw);
            callData[1] = withdrawAssetCalldata;
        }
        ...
        // Transfer Assets
        if(usdtWithdraw!=0){
            require(IERC20(asset).transfer(_address, usdtWithdraw), "Transfer asset from Compound failed");
        }

        emit Repay(usdcRepay, usdtWithdraw);
    }
    ...
    function getUSDTWithdrawWithSlippage(uint256 repayUsdcAmount, uint256 _repaySlippage) public view returns (uint256) {
